...
from shared.utils import LoggingClient, require_permission, InsufficientPermissionError
...
def create_job_entries(jobs, project, sub, missing_to_ref):
    job_entries = []
    for job in jobs:
        source_vcf_key = (
    ...
    for i in range(0, len(job_entries), 10):
        ...
        try:
            response = sqs_client.send_message_batch(
                QueueUrl=PGXFLOW_BATCH_SUBMIT_QUEUE_URL, Entries=message_batch
            )
