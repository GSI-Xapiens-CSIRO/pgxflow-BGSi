...
# /pipeline_pharmcat
#
resource "aws_api_gateway_resource" "pipeline_pharmcat" {
  rest_api_id = var.pgxflow-api-gateway-id
  parent_id   = var.pgxflow-api-gateway-root-resource-id
  path_part   = "pipeline_pharmcat"
}

# 
# /pipeline_pharmcat/submit
# 
...
resource "aws_api_gateway_integration" "submit-patch" {
  ...
  http_method             = aws_api_gateway_method.submit-patch.http_method
  type                    = "AWS_PROXY"
  uri                     = module.lambda-initPharmcat.lambda_function_invoke_arn
...
resource "aws_lambda_permission" "APIInitPharmcat" {
  statement_id  = "AllowAPIInitPharmcatInvoke"
  action        = "lambda:InvokeFunction"
  function_name = module.lambda-initPharmcat.lambda_function_name
